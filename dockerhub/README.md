...
Then deploy with the non-dev compose from `dockerhub/with-makemkv/docker-compose.yml` (local `linux-video-encoder:latest` image),
or use the Portainer stack in `portainer/docker-compose.yml`.
