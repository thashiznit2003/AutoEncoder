...
# Setup Samba share "smbstaging" and restart the stack.
# Defaults:
#   BASE_DIR=/linux-video-encoder
#   REPO_DIR=$BASE_DIR/AutoEncoder/linux-video-encoder
#   SMB_STAGING_DIR=$REPO_DIR/SMBStaging
...
if [ "${EUID:-$(id -u)}" -ne 0 ]; then
  ...
  if ! command -v sudo >/dev/null 2>&1; then
    printf '[smbstaging] sudo not found; please run as root.\n' >&2
    exit 1
  fi
fi
...
SMB_STAGING_DIR="${SMB_STAGING_DIR:-$REPO_DIR/SMBStaging}"
SMB_USER="${SMB_USER:-}"
SMB_PASS="${SMB_PASS:-}"

log() { printf '[smbstaging] %s\n' "$*"; }
...
setup_samba() {
  ...
  log "Preparing staging directory at $SMB_STAGING_DIR ..."
  $SUDO mkdir -p "$SMB_STAGING_DIR"
  $SUDO chown -R "$SMB_USER":"$SMB_USER" "$SMB_STAGING_DIR"
  $SUDO chmod -R 775 "$SMB_STAGING_DIR"
  ...
  if samba_share_exists smbstaging; then
    log "Samba share smbstaging already exists; leaving smb.conf unchanged."
  else
    cat <<CONFIG | $SUDO tee -a /etc/samba/smb.conf >/dev/null

[smbstaging]
   path = $SMB_STAGING_DIR
   browseable = yes
   read only = no
   guest ok = no
...
CONFIG
    ...
    log "Samba share smbstaging configured. Access smb://<host>/smbstaging with user '$SMB_USER'."
  fi
}
