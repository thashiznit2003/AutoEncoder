...
<body>
  ...
  <script>
    ...
    const mobileToggle = document.getElementById("mobile-nav-toggle");
    const mobileItems = document.getElementById("mobile-nav-items");
    const desktopItems = document.getElementById("desktop-nav-items");
    ...
    function updatePanelVisibility() {
      const panels = Array.from(document.querySelectorAll(".panel[data-panel-title]"));
      if (!panels.length) return;
      const visiblePanels = panels.filter((panel) => panel.dataset.hiddenByAdvanced !== "1");
      if (!visiblePanels.length) return;
      if (!activePanelId || !visiblePanels.some((panel) => panel.id === activePanelId)) {
        activePanelId = visiblePanels[0].id;
      }
      panels.forEach((panel) => {
        if (panel.dataset.hiddenByAdvanced === "1") {
          panel.style.display = "none";
          return;
        }
        panel.style.display = panel.id === activePanelId ? "" : "none";
      });
      [mobileItems, desktopItems].forEach((container) => {
        if (!container) return;
    ...
    function buildPanelNav() {
      const panels = Array.from(document.querySelectorAll(".panel[data-panel-title]")).filter((panel) => panel.dataset.hiddenByAdvanced !== "1");
      [mobileItems, desktopItems].forEach((container) => {
        if (!container) return;
        container.innerHTML = "";
        panels.forEach((panel) => {
    ...
    function applyAdvancedMode(enabled) {
      document.querySelectorAll(".panel[data-advanced='1']").forEach((panel) => {
        panel.dataset.hiddenByAdvanced = enabled ? "0" : "1";
      });
      buildPanelNav();
    }
    ...
    document.getElementById("hb-save").addEventListener("click", async (e) => {
      ...
      const body = {
        ...
        handbrake_dvd: { encoder: document.getElementById("hb-encoder").value, quality: qDvd, extension: ext, audio_mode: audioMode, audio_bitrate_kbps: audioBitrate, audio_encoder: audioEncoder, audio_mixdown: audioMix, audio_samplerate: audioRate, audio_drc: audioDrc, audio_gain: audioGain, audio_offset_ms: audioOffset, audio_lang_list: audioLang, audio_track_list: audioTracks, audio_all: audioAll, subtitle_mode: subtitleMode, video_bitrate_kbps: targetBitrate, two_pass: twoPass },
        handbrake_br: { encoder: document.getElementById("hb-encoder").value, quality: qBr, extension: ext, audio_mode: audioMode, audio_bitrate_kbps: audioBitrate, audio_encoder: audioEncoder, audio_mixdown: audioMix, audio_samplerate: audioRate, audio_drc: audioDrc, audio_gain: audioGain, audio_offset_ms: audioOffset, audio_lang_list: audioLang, audio_track_list: audioTracks, audio_all: audioAll, subtitle_mode: subtitleMode, video_bitrate_kbps: targetBitrate, two_pass: twoPass }
      };
